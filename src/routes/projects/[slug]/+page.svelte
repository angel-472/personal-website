...
<svelte:head>
  ...
  <meta name="description" content={data.post.metadata.excerpt ?? "A project post on Diaza's personal website."} />
</svelte:head>

<!-- Todo: Add heading that will render the post title, date, cover image, etc -->
<section class="lg:px-24 xl:px-32 text-zinc-100">
  <Link href="/projects" isUnderlined={false} class="inline-block mb-4 text-zinc-400">
    &larr; Back to Projects
  </Link>
  <h1 class="text-4xl font-bold mb-2">{data.post.metadata.title}</h1>
  <!-- Links -->
  <div class="flex items-center gap-4 mb-2 text-lg">
    {#if data.post.metadata.githubUrl}
      <Link isUnderlined={false} class="flex items-center gap-1 text-zinc-100" href={data.post.metadata.githubUrl}>Source Code <ExternalLink size={16} /></Link>
    {/if}
    {#if data.post.metadata.demoUrl}
      <Link isUnderlined={false} class="flex items-center gap-1 text-zinc-100" href={data.post.metadata.demoUrl}>Live Demo <ExternalLink size={16} /></Link>
    {/if}
  </div>
  <!-- Post details -->
...
<style>
  ...
  /* Links */
  .blog-post :global(a) {
    @apply text-cyan-200 underline underline-offset-4 decoration-dashed transition-all duration-200 hover:font-bold;
  }
